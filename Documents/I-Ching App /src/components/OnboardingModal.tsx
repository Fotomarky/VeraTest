...
const OnboardingModal: React.FC<Props> = ({ isOpen, onComplete }) => {
  ...
  const handleAgeNext = () => {
    ...
    setStep('welcome');
    // Small delay so user sees the welcome screen before modal closes
    setTimeout(() => onComplete({ name: name.trim(), age }), 1600);
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm animate-fade-in px-6">
      <div className="w-full max-w-[380px] bg-[#0f1a14] border border-[#d4af37]/30 rounded-2xl px-8 py-10 shadow-2xl flex flex-col items-center gap-6">
        ...
        <div className="flex gap-2">
          {(['name', 'age', 'welcome'] as Step[]).map((stepKey, i) => (
            <div
              key={stepKey}
              className={`h-1.5 rounded-full transition-all duration-500 ${
                step === stepKey ? 'w-8 bg-[#d4af37]' : i < ['name', 'age', 'welcome'].indexOf(step) ? 'w-4 bg-[#d4af37]/50' : 'w-4 bg-white/15'
              }`}
            />
          ))}
        </div>
